...
- **Trang quản lý sản phẩm (`quan-ly-san-pham.html`) — ĐÃ VIẾT XONG CODE (2026-07-22) NHƯNG CHƯA ĐƯỢC TEST TRÊN SHEET/DRIVE THẬT** — việc cần làm ngay khi quay lại phiên sau:
  ...
  - **Xác nhận đã gặp lỗi "You do not have permission to call DriveApp..."** ở lần lưu sản phẩm có ảnh đầu tiên (2026-07-22, test thật) — đúng như dự đoán, vì đây là lần đầu code gọi tới `DriveApp` (scope Drive chưa từng được cấp quyền trước đó). Đã thêm hàm `testDriveAccess()` (KHÔNG có dấu `_`, hiện trong dropdown Run) chuyên để xin quyền Drive cho rõ ràng — **PHẢI chọn đúng hàm này** ở dropdown rồi bấm Run (chạy nhầm `doPost`/`saveOrder` sẽ KHÔNG xin quyền được gì vì các hàm đó lỗi/thoát trước khi chạm tới DriveApp). Chạy xong không lỗi thì xem "Nhật ký thực thi" thấy dòng "OK - Thư mục Drive đã sẵn sàng" là xong.
